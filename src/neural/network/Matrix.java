...
public class Matrix {
	...
	public static Matrix average(List<Matrix> listMatrix) {
		Matrix temp = new Matrix(listMatrix.get(0).rows, listMatrix.get(0).cols);

		for (Matrix m : listMatrix) {
			for (int i = 0; i < temp.rows; i++) {
				for (int j = 0; j < temp.cols; j++) {
		...
		for (int i = 0; i < temp.rows; i++) {
			for (int j = 0; j < temp.cols; j++) {
				temp.data[i][j] = temp.data[i][j] / listMatrix.size();
			}
		}
